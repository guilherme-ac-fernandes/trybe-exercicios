...
const externalCep = require('../models/externalCep');
...
const findAddressByCep = async (cepInput) => {
  ...
  if (cep) return { code: 200, cep };
  const cepFromApi = await externalCep(cepInput);
  if (cepFromApi.erro) {
    return { error: { code: "notFound", message: "CEP não encontrado" } };
  }
  return CepModels.createCep(cepFromApi)
};
